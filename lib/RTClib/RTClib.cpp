...
void RTC_DS1307::adjust(const struct tm &t)
{
  Wire.beginTransmission(DS1307_ADDRESS);
  Wire._I2C_WRITE((byte)0); // start at location 0
  Wire._I2C_WRITE(bin2bcd(t.tm_sec));
  Wire._I2C_WRITE(bin2bcd(t.tm_min));
  Wire._I2C_WRITE(bin2bcd(t.tm_hour));
  Wire._I2C_WRITE(bin2bcd(0));
  Wire._I2C_WRITE(bin2bcd(t.tm_mday));
  Wire._I2C_WRITE(bin2bcd(t.tm_mon + 1));
  Wire._I2C_WRITE(bin2bcd(t.tm_year));
  Wire.endTransmission();
}
/**************************************************************************/
/*!
